...
def create_ocr_visualization(mmif, view):
        ...
        try:
            ...
            if str(anno.at_type).endswith('BoundingBox'):
                ...
                tf = tempfile.NamedTemporaryFile(
                    prefix="/app/static/tmp/", suffix=".jpg", delete=False)
                ...
                if frame_num in frames.keys():
                    ...
                    frames[frame_num] = {"id": id, "boxes": [
                        box], "text": [], "bb_ids": [box_id]}
    ...
    return render_ocr(frames, alignments, text_docs, fps)


def render_ocr(frames, alignments, text_docs, fps):
    """Iterate through frames and display the contents/alignments"""
    text = "<pre>"
    for frame_num, frame in frames.items():
        text += f'<div class="ocr">{html_img(frame["id"] , boxes = frame["boxes"], id = frame["id"])}<h4>frame: {frame_num}\n'
        if fps:
            secs = int(frame_num/fps)
            text += f'timestamp: <a class="timestamp" onclick="SetCurTime({secs})">{datetime.timedelta(seconds=secs)}</a>\n'
        text += 'text detected:\n'
        for box_id in frame["bb_ids"]:
            text_id = alignments[box_id]
            text += f'  {text_docs[text_id]}\n'
        text += '</h4></div>'
